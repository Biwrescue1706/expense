...
exports.create = async (data) => {
    ...
    if (users.find(u => u[1]?.toLowerCase() === username.toLowerCase())) {
        throw new Error("Username นี้ถูกใช้งานแล้ว");
    }

    if (users.find(u => u[2]?.toLowerCase() === email.toLowerCase())) {
        throw new Error("Email นี้ถูกใช้งานแล้ว");
    }

    if (users.find(u => u[9] === phone)) {
        throw new Error("เบอร์โทรศัพท์นี้ถูกใช้งานแล้ว");
    }

    if (users.find(u => u[10] === citizenId)) {
